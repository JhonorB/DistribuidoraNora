...
function renderizarCheckout() {
    ...
    carrito.forEach((producto, index) => {
        ...
        // Tomar la primera imagen del array si existe, o un placeholder si no
        const imagenSrc = (producto.imagenes && producto.imagenes.length > 0)
            ? producto.imagenes[0]
            : (producto.imagen || "ruta/a/imagen-placeholder.jpg");

        const fila = document.createElement("tr");
        fila.innerHTML = `
            <td><img src="${imagenSrc}" alt="${producto.nombre}" /></td>
